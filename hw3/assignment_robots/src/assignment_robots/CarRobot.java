...
public class CarRobot {
	...
	protected double initial[][] = { { 10, 10 }, { 20, 0 }, { 10, -10 },
			{ -10, -10 }, { -10, 10 } };
	...
	public double[][] get() {
		...
		for (int i = 0; i < rn.length; i++) {
			current[0] = initial[i][0] * Math.cos(s.getTheta()) - initial[i][1]
					* Math.sin(s.getTheta()) + s.getX();
			current[1] = initial[i][0] * Math.sin(s.getTheta()) + initial[i][1]
					* Math.cos(s.getTheta()) + s.getY();
			rn[i][0] = current[0];
			rn[i][1] = current[1];
		}
		return rn;
