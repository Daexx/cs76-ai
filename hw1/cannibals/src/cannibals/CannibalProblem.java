		...
		public ArrayList<UUSearchNode> getSuccessors() {
			...
			if (state[2] != 0) {
				for (int i = Math.min(state[2] * BOAT_SIZE, state[0]); i >= 0; i--) {
					for (int j = Math.min(state[2] * BOAT_SIZE - i, state[1]); j >= 0; j--) {
						if (i + j < totalBoats)
							continue;
				...
				for (int i = Math.min(state2 * BOAT_SIZE, state0); i >= 0; i--) {
					for (int j = Math.min(state2 * BOAT_SIZE - i, state1); j >= 0; j--) {
						if (i + j < totalBoats)
							continue;
						CannibalNode tryNode = new CannibalNode(state[0] + i,
